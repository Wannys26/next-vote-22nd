...
export const checkIdDuplicate = async (loginId: string): Promise<CheckAvailableResponse> => {
  const response = await axiosInstance.get<CheckAvailableResponse>(`/check/login-id?value=${loginId}`);
  return response.data;
};

export const checkEmailDuplicate = async (email: string): Promise<CheckAvailableResponse> => {
  const response = await axiosInstance.post<CheckAvailableResponse>('/check/email', { value: email });
  return response.data;
};
