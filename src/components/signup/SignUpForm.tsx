...
const SignUpForm = () => {
  ...
  const {
    ...
    isFetching: isIdChecking,
  } = useCheckIdDuplicateQuery(form.userId, { enabled: false });
  const {
    data: emailCheckData,
    refetch: refetchEmailCheck,
    isFetching: isEmailChecking,
  } = useCheckEmailDuplicateQuery(form.userEmail, { enabled: false });
  ...
  return (
    ...
    >
      ...
      <p className="text-center text-gray-700 mb-8 md:mb-6 text-body-2-semibold">
        투표 시스템에 가입하고 투표에 참여하세요
      </p>
      {/* 팀 선택 버튼 */}
      ...
      <div className="flex h-12 w-full mb-6 justify-between">
        <button
          type="button"
          className={`w-[138px] px-0 py-3 md:w-[290px] md:py-3 rounded-[14px] text-body-1-semibold cursor-pointer transition-all duration-150 ${
            form.selectedTeam === 'FRONT-END' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'
          }`}
        ...
        <button
          type="button"
          className={`w-[138px] px-0 py-3 md:w-[290px] md:py-3 rounded-[14px] text-body-1-semibold cursor-pointer transition-all duration-150 ${
            form.selectedTeam === 'BACK-END' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'
          }`}
      ...
      <div className="flex flex-col w-full mb-6">
        <Select
          options={
            form.selectedTeam === 'FRONT-END' ? frontendNames : form.selectedTeam === 'BACK-END' ? backendNames : []
          }
      ...
      <div className="flex w-full gap-2 flex-nowrap">
        ...
        />
        <CheckButton disabled={form.userId.length < 6} onClick={handleUserIdCheck} className="px-4 py-3">
          중복확인
        </CheckButton>
      </div>
      <MessageBlock
      ...
      <div className="flex flex-col w-full">
        ...
        <MessageBlock
          ...
          checkError={
            emailCheckRequested &&
            !isEmailChecked &&
            (!emailCheckData || (emailCheckData && emailCheckData.result.available)) &&
            !isEmailChecking
          }
